...
public interface ItemCountRepository  extends JpaRepository<ItemCount, Long> {
	...
	public Optional<ItemCount> findByQrcodeAndFamilyIdAndStorageType(@Param("qrCode") String qrCode, @Param("familyId") long familyId, @Param("storageName") String storageName);
	...
	@Transactional
	@Query(value = 
			  "INSERT INTO item_count (item_id, storage_id, count) SELECT item_id, storage_id, :amount as count FROM item, storage WHERE (qr_code = :qrCode) AND storagetype_id = (SELECT storagetype_id FROM storage_type WHERE name = :storageName) AND family_id = :familyId"
			, nativeQuery = true)
	
	public int insertItemCount(@Param("qrCode") String qrCode, @Param("familyId") long familyId, @Param("storageName") String storageName, @Param("amount") int amount);
}
